...
它的核心目标是做成一个支持多种 provider 和多种 API 接入形式的编码 Agent，包括 OpenAI Responses 兼容接口、Anthropic Messages 兼容接口，以及可扩展的自定义 provider 配置。

当前可运行入口是 [project/cumt_code.mjs](project/cumt_code.mjs)。仓库中的 `src/` 主要作为参考源码保留，不是当前打包发布的入口。
...
CUMT Code 的定位是“矿大的开源 Code Agent，支持接入多个 provider，而且切换时不需要重启”。它当前支持：
...
![CUMT Code Setup](figures/setup.png)
